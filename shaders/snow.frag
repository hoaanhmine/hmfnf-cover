...
void main()
{
	...
	for (int i = STARTING_LAYERS; i < LAYERS; i++) {
        ...
        q += vec2(q.y * ((WIDTH * (pixely ? 1.5 : 1.0)) * mod(fi * 7.238917, 1.0) - (WIDTH * (pixely ? 1.5 : 1.0)) * 0.5) + (((SPEED) * ((float(LAYERS) - fi) * 0.2)) * (time * 0.4)), SPEED * time / (1.0 + fi * DEPTH * 0.03));
        vec3 n = vec3(floor(q), 31.189 + fi);
		vec3 m = floor(n)*.00001 + fract(n);
		vec3 mp = (31415.9+m)/fract(p*m);
		vec3 r = fract(mp);
		vec2 s = abs(mod(q,1.)-.5+.9*r.xy-.45);
		s += .01*abs(2.*fract(10.*q.yx)-1.0); 

...
vec4 getVideo(vec2 uv)
{
    ...
    if (moveScreenX) {
        look.x = look.x + (sin(look.y*10.0 + iTime)/50.0 * onOff((4.0 * timeMulti),(4.0 * timeMulti),(.3 * timeMulti)) * (1.0 + cos(iTime*80.0)) * window) * (glitchModifier * 2.0);
    }
    
    float vShift = 0.4 * onOff((2.0 * timeMulti),(3.0 * timeMulti),(.9 * timeMulti)) * (sin((iTime * effectMulti)) * sin((iTime * effectMulti) * 20.0) + (0.5 + 0.1 * sin((iTime * effectMulti) * 200.0) * cos((iTime * effectMulti))));
    if (moveScreenFullY) {

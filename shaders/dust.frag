...
vec3 hash33(vec3 p){ 
    ...
    return fract(vec3(2097152.0, 262144.0, 32768.0)*n)*2. - 1.; // return fract(vec3(64, 8, 1)*32768.0*n)*2.-1.; 
}
...
void main() {
	...
	for (int i = STARTING_LAYERS; i < LAYERS; i++) {
		...
		q += vec2(q.y * ((WIDTH * (pixely ? 1.5 : 1.0)) * mod(fi * 7.238917, 1.0) - (WIDTH * (pixely ? 1.5 : 1.0)) * 0.5) + (((SPEED) * ((float(LAYERS - i)) * 0.2)) * (time * 0.4)) * -0.6, - (SPEED * time / (1.0 + fi * DEPTH * 0.03)));

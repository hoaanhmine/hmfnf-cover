...
float simulate_fwidth(float value, vec2 fragCoord) {
    ...
    float val_x1 = value + (fragCoord.x + dx.x) / openfl_TextureSize.x;
    float val_x2 = value - (fragCoord.x - dx.x) / openfl_TextureSize.x;
    float val_y1 = value + (fragCoord.y + dy.y) / openfl_TextureSize.y;
    float val_y2 = value - (fragCoord.y - dy.y) / openfl_TextureSize.y;
...
float func(vec2 p){
    float n = tetraNoise(vec3(p.x*4., p.y*4., 0.0) - vec3(0.0, .25, .5)*time);

...
function onCreatePost()
	for event = 0, getProperty('eventNotes.length') - 1 do
      ...
      local time = getPropertyFromGroup('eventNotes', event, 'value1') 
		if pauser then
         local offse = (time * 0 * scrollSpeed) * (downscroll and 1 or -1)
   ...
   if name == "pauser" then 
      ...
      for i = 0, getProperty('notes.length') - 1 do
         if getPropertyFromGroup('notes', i, 'mustPress') then
            setPropertyFromGroup('notes', i, 'y', getPropertyFromGroup('notes', i, 'y') + (time * 289 * scrollSpeed) * (downscroll and 1 or -1))
